...
dev = usb.core.find(idVendor=0x04b0, idProduct=0x0421)
if dev is None:
...
# Write to RAM location 0x1c0d0000 value 0x08000000 (likely setting bias point to 128 so that all left tail is
...
dev.write(0x02, b"\x18\x00\x00\x00\x01\x00\x31\xfd\x06\x00\x00\x00\x1c\x0d\x00\x00\x04\x00\x00\x00\x00\x00\x00\x00", 100)
# ContainerLength=16    ContainerType=DataBlock       Code=0xfd31    TransactionID=6    Payload=08000000
dev.write(0x02, b"\x10\x00\x00\x00\x02\x00\x31\xfd\x06\x00\x00\x00\x08\x00\x00\x00", 100)
rx_len = dev.read(0x81, rx_buff, 400)
